...
NOD, WNOD, RNOD, WMO, RAD, PLC, CCCC, WIGOS = (
    {None: None},
    {None: None},
    {None: None},
    {None: None},
    ...
    for country in list(DB):
        ...
        for radar in list(country):
            ...
            if wmo not in ("00000", None):
                NOD[wmo] = nod
                SOURCE[nod] = "NOD:%s" % nod
                if wmo:
                    SOURCE[nod] += ",WMO:%s" % wmo
                if rad:
                    SOURCE[nod] += ",RAD:%s" % rad
                if plc:
                    SOURCE[nod] += ",PLC:%s" % plc
                if wigos:
                    SOURCE[nod] += ",WIGOS:%s" % wigos
            else:
                SOURCE[nod] = "NOD:%s" % nod
                if rad:
                    SOURCE[nod] += ",RAD:%s" % rad
                if org and wmo != None:
                    SOURCE[nod] += ",ORG:%s" % org
                if plc:
                    SOURCE[nod] += ",PLC:%s" % plc
                if wigos:
                    SOURCE[nod] += ",WIGOS:%s" % wigos
...
def text2Element(filename, xmlfile=ODIM_SOURCE_FILE):
    ...
    for L in LINES:
        ...
        if len(l) == 3:
            ...
            E.attrib["CCCC"] = CCCC
            R = ET.Element(nod, {"wmo": wmo, "rad": "%s%s" % (rad_prefix, nr), "plc": plc.decode('utf-8')})
            E.append(R)
        else:
...
class ODIM_Source:
    ...
    def split_source(self):
        ...
        for s in split:
            ...
            if prefix == b'wmo':
                self.wmo = value.decode(UTF8)  # Keep this as a string!
            elif prefix == b'rad':
                self.rad = value.decode(UTF8)
            elif prefix == b'plc':
                self.plc = value.decode(UTF8)
            elif prefix == b'nod':
                self.nod = value.decode(UTF8)
            elif prefix == b'org':
                self.org = value.decode(UTF8)
            elif prefix == b'cty':
                self.cty = value.decode(UTF8)
            elif prefix == b'cmt':
                self.cmt = value.decode(UTF8)
            elif prefix == b'wigos':
                self.wigos = value.decode(UTF8)
