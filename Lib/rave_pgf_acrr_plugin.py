...
def get_backup_gra_coefficient(db, agedt, nowdt, identifier=None):
    ...
    logger.warning(
        "Could not aquire coefficients newer than %s, defaulting to climatologic" % agedt.strftime("%Y%m%d %H:%M:%S")
    )
...
def generate(files, arguments):
    ...
    for fname in files:
        ...
        if (
            obj.xscale != img.xscale
            or obj.yscale != img.yscale
            or obj.projection.definition != img.projection.definition
        ):
