...
class RavePGF:
    ...
    def register(self, name, module, function, Help="", strings="", ints="", floats="", seqs=""):
        ...
        self._algorithm_registry.register(
            name, module, function, Help=Help, strings=strings, ints=ints, floats=floats, seqs=seqs
        )
    ...
    def generate(self, algorithm, files, arguments):
        ...
        try:
            ...
            self.logger.info(
                f"[{self.name}] RavePGF.generate: ID={jobid} added job with algorithm={algorithm} to runner queue."
            )
        ...
        try:
            ...
            for item in items:
                result[item[0]] = {
                    ...
                    "definition": " ".join(item[1].definition),
                }
        except Exception:
    ...
    def _generate(self, algorithm, files, arguments):
        ...
        self.logger.info(
            f"[{self.name}] RavePGF._generate: Request ID={self._jobid}, algorithm={algorithm}, {arguments}."
        )
        ...
        try:
            ...
            verified = rave_pgf_verify.verify_generate_args(arguments, algorithm_entry)
            if not verified:
            ...
            self.logger.info(
                f"[{self.name}] RavePGF._generate: Done running ID={self._jobid} algorithm={algorithm}, outfile={outfile}."
            )
            ...
            self.logger.exception(
                f"[{self.name}] RavePGF._generate: Request with ID={self._jobid}, algorithm={algorithm} failed."
            )
        ...
        self.logger.info(
            f"[{self.name}] RavePGF._generate: Request with ID={self._jobid} returning '{result}' after {totaltime} ms."
        )
        return result
