...
def get_backup_gra_coefficient(db, maxage):
    ...
    logger.warn(
        "Could not aquire coefficients newer than %s, defaulting to climatologic" % maxage.strftime("%Y%m%d %H:%M:%S")
    )
...
def calculate_gra_coefficient(distancefield, interval, adjustmentfile, etime, edate, acrrproduct, db):
    ...
    if len(points) > 2:
        try:
            ...
            if math.isnan(a) or math.isnan(b) or math.isnan(c):
                logger.error(
                    "A/B or C for %s %s is not a number: %f,%f,%f" % (acrrproduct.date, acrrproduct.time, a, b, c)
                )
                return
        except Exception:
    ...
    # If we come here,
    # store the coefficients in the database so that we can search for them when applying the coefficients
