...
from rave_defines import (
    RAVE_TILE_COMPOSITING_PROCESSES,
    RAVE_TILE_COMPOSITING_TIMEOUT,
...
class tiled_area_definition(object):
    ...
    def __repr__(self):
        return "<%s, scale=%f * %f, size=%d * %d, extent=%s />" % (
            self.pcsdef,
            self.xscale,
            self.yscale,
            self.xsize,
...
class multi_composite_arguments(object):
    ...
    def generate(self, dd, dt, tid):
        ...
        logger.debug(
            f"[{mpname}] multi_composite_arguments.generate: Generating composite tile={self.area_definition.id}"
        )
        ...
        if result == None:
            ...
            logger.info(
                f"[{mpname}] multi_composite_arguments.generate: No composite for tile={self.area_definition.id} could be generated."
            )
            return (tid, None, totaltime)
        else:
            logger.debug(
                f"[{mpname}] multi_composite_arguments.generate: Finished generating composite for tile={self.area_definition.id}."
            )
...
class tiled_compositing(object):
    ...
    def _fetch_file_objects(self):
        self.logger.debug(
            f"[{self.mpname}] tiled_compositing.fetch_file_objects: Fetching (and processing) {len(self.compositing.filenames)} files for tiled compositing"
        )
        ...
        if self.preprocess_qc:
            ...
            try:
                ...
                self.logger.exception(
                    f"[{self.mpname}] tiled_compositing.fetch_file_objects: Failed to create temporary files. will not preprocess qc."
                )

        self.logger.debug(
            f"[{self.mpname}] tiled_compositing.fetch_file_objects: Finished fetching (and processing) {len(self.compositing.filenames)} files for tiled compositing"
        )
    ...
    def _fetch_file_objects_mp(self):
        self.logger.info(
            f"[{self.mpname}] tiled_compositing.fetch_file_objects: Fetching (and processing) %d files for tiled compositing"
            % len(self.compositing.filenames)
        )
        args = []
        ...
        if self.mp_process_qc_split_evenly and self.number_of_quality_control_processes > 0:
            ...
            for x in range(0, nrfiles, nrslices):
                args.append(
                    (
                        self.compositing.filenames[x : x + nrslices],
                        self.compositing.detectors,
                        self.compositing.reprocess_quality_field,
                        self.compositing.ignore_malfunc,
                    )
                )
        else:
        ...
        for r in results[0]:
            ...
            if r[1] == False:
                self.logger.info(
                    f"[{self.mpname}] tiled_compositing.fetch_file_objects: quality control processing of %s failed."
                    % str(r[2])
                )
    ...
    def _add_files_to_argument_list(self, args, tiled_areas):
        self.logger.debug(
            f"[{self.mpname}] tiled_compositing._add_files_to_argument_list: Distributing polar objects among {len(args)} tiles"
        )
        ...
        for idx in range(len(args)):
            self.logger.info(
                f"[{self.mpname}] tiled_compositing._add_files_to_argument_list: Tile {args[idx][0].area_definition.id} contains  {len(args[idx][0].filenames)} files and dimensions {args[idx][0].area_definition.xsize} x {args[idx][0].area_definition.ysize}"
            )

        self.logger.debug(
            f"[{self.mpname}] tiled_compositing._add_files_to_argument_list: Finished splitting polar object"
        )
    ...
    def _ensure_date_and_time_on_args(self, args):
        ...
        if dtstr is None or ddstr is None:
            self.logger.info(
                f"[{self.mpname}] tiled_compositing._ensure_date_and_time_on_args: Could not determine any date and time string"
            )
            return False
    ...
    def generate(self, dd, dt, area=None):
        ...
        if all_files_malfunc:
            self.logger.info(
                "[{self.mpname}] tiled_compositing.generate: Content of all provided files were marked as 'malfunc'. Since option 'ignore_malfunc' is set, no composite is generated!"
            )
            return None
        ...
        if len(results) == len(args):
            ...
            for a in args:
                if not a[3] in processed_areas:
                    self.logger.error(
                        f"[{self.mpname}] tiled_compositing.generate: No answer from subprocess when generating composite tile with areaid: {a[3]}"
                    )
                    ...
                    if RAVE_TILE_COMPOSITING_ALLOW_MISSING_TILES:
                        ...
                        raise RuntimeError(
                            f"No answer from subprocess when generating composite tile with areaid: {a[3]}"
                        )
        ...
        if len(results) > 0:
            for v in results[0]:
                self.logger.info(
                    f"[{self.mpname}] tiled_compositing.generate: Tile with areaid: {v[0]} took {v[2]} seconds to process"
                )
        ...
        try:
            for v in results[0]:
                ...
                if tile_file == None:
                    self.logger.warning(
                        f"[{self.mpname}] tiled_compositing.generate: No partial composite for tile area {v[0]} was created. This tile will therefore not be included in complete composite."
                    )
                else:
            ...
            self.logger.debug(
                f"[{self.mpname}] tiled_compositing.generate: Combining {len(objects)} tiles into one composite for area {area}."
            )
            ...
            self.logger.info(
                f"[{self.mpname}] tiled_compositing.generate: Tiled compositing took {totaltime} ms to execute."
            )
            ...
            if self._do_remove_temporary_files:
                for fname in self.compositing.filenames:
                    try:
                        ...
                        logger.warning(
                            f"[{self.mpname}] tiled_compositing.generate: Failed to remove temporary file: {fname}"
                        )
...
def comp_generate(args):
    ...
    starttime = time.time()
    logger.info(
        f"[{mpname}] tiled_compositing.comp_generate. Starting generation of tile {args[3]} - {args[1]}{args[2]}"
    )
    try:
...
def execute_quality_control(args):
    ...
    try:
        ...
        logger.info(
            f"[{mpname}] tiled_compositing.execute_quality_control: Starting QC of {len(comp.filenames)} objects."
        )
...
if __name__ == "__main__":
    comp = compositing.compositing()
    comp.filenames = [
        "/projects/baltrad/rave/test/pytest/fixtures/pvol_seang_20090501T120000Z.h5",
    ...
    comp.filenames = [
        "/projects/baltrad/baltrad-test/fixtures4/seang_pvol_20140220T1300Z.h5",
