...
# This functionallity relies on the use of multiprocessing and storing intermediate files representing tiles.
# When the other processes are finishied with the processing, the result is put back into the full area again.
#
...
class tiled_compositing(object):
    ...
    def _fetch_file_objects(self):
        ...
        self.logger.info(
            "Finished fetching (and processing) %d files for tiled compositing" % len(self.compositing.filenames)
        )
    ...
    def _fetch_file_objects_mp(self):
        self.logger.info(
            "MP Fetching (and processing) %d files for tiled compositing" % len(self.compositing.filenames)
        )
        args = []
    ...
    def _add_files_to_argument_list(self, args, tiled_areas):
        ...
        for idx in range(len(args)):
            self.logger.info(
                ...
                % (
                    args[idx][0].area_definition.id,
                    len(args[idx][0].filenames),
    ...
    def generate(self, dd, dt, area=None):
        ...
        if all_files_malfunc:
            self.logger.info(
                "Content of all provided files were marked as 'malfunc'. "
            ...
            for a in args:
                if not a[3] in processed_areas:
                    ...
                    if RAVE_TILE_COMPOSITING_ALLOW_MISSING_TILES:
                        ...
                        raise RuntimeError(
                            "No answer from subprocess when generating composite tile with areaid: %s" % a[3]
                        )
        ...
        try:
            for v in results[0]:
                ...
                if tile_file is None:
                    self.logger.warn(
                        "No partial composite for tile area %s was created. "
                        "This tile will therefore not be included in complete composite.",
                        v[0],
                    )
                else:
            ...
            if results is not None:
                for v in results[0]:
                    if v is not None and v[1] is not None and os.path.exists(v[1]):
                        try:
                            os.unlink(v[1])
                        except Exception:
