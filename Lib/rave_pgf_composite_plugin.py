    ...
    if "interpolation_method" in args.keys():
        ...
        if interpolation_method != "NEAREST_VALUE":
            if comp.quantity == "DBZH":
                ...
                logger.info(
                    f"[{mpname}] rave_pgf_composite_plugin.generate: Interpolation method {interpolation_method} is currently only supported for quantity DBZH. Provided quantity: {comp.quantity}. No composite generated."
                )
                return None
