...
class compositing(object):
    ...
    def _generate(self, dd, dt, area=None):
        ...
        self.logger.debug(
            f"[{self.mpname}] compositing.generate: Generating composite with date and time {dd}T{dt} for area {area}"
        )
        ...
        if all_files_malfunc:
            self.logger.info(
                f"[{self.mpname}] compositing.generate: Content of all provided files were marked as 'malfunc'. Since option 'ignore_malfunc' is set, no composite is generated!"
            )
            return None
        ...
        self.logger.debug(
            f"[{self.mpname}] compositing.generate: Quality controls for composite generation: {qccontrols}"
        )

        if len(objects) == 0:
            self.logger.info(
                f"[{self.mpname}] compositing.generate: No objects provided to the composite generator. No composite will be generated!"
            )
            return None
        ...
        if self.applygra:
            if not "se.smhi.composite.distance.radar" in qfields:
                self.logger.info(
                    f"[{self.mpname}] compositing.generate: Trying to apply GRA analysis without specifying a quality plugin specifying the se.smhi.composite.distance.radar q-field, disabling..."
                )
            else:
                if self.verbose:
                    self.logger.info(
                        f"[{self.mpname}] compositing.generate: Applying GRA analysis (ZR A = {self.zr_A}, ZR b = {self.zr_b})"
                    )
    ...
    def set_interpolation_method_from_string(self, methstr):
        if methstr.upper() == "NEAREST_VALUE":
            ...
            raise ValueError(
                "Only supported interpolation methods are NEAREST_VALUE, LINEAR_HEIGHT, LINEAR_RANGE, LINEAR_AZIMUTH, LINEAR_RANGE_AND_AZIMUTH, LINEAR_3D, QUADRATIC_HEIGHT or QUADRATIC_3D"
            )
    ...
    def set_quality_control_mode_from_string(self, modestr):
        if modestr.lower() not in [QUALITY_CONTROL_MODE_ANALYZE, QUALITY_CONTROL_MODE_ANALYZE_AND_APPLY]:
            raise ValueError(
                "Invalid quality control mode (%s), only supported modes are analyze_and_apply or analyze"
                % modestr.lower()
            )
    ...
    def quality_control_objects(self, objects):
        ...
        for k in objects.keys():
            ...
            for d in self.detectors:
                ...
                if p != None:
                    process_result = p.process(obj, self.reprocess_quality_field, self.quality_control_mode, self.detector_arguments)
    ...
    def fetch_objects(self):
        ...
        for fname in self.filenames:
            ...
            if not is_scan and not is_pvol:
                self.logger.warning(
                    f"[{self.mpname}] compositing.fetch_objects: Input file {fname} is neither polar scan or volume, ignoring."
                )
                continue
            ...
            if self.ignore_malfunc:
                ...
                if obj is None:
                    self.logger.info(
                        f"[{self.mpname}] compositing.fetch_objects: Input file {fname} detected as 'malfunc', ignoring."
                    )
                    malfunc_files += 1
                    continue
            ...
            if is_scan:
                self.logger.debug(
                    f"[{self.mpname}] compositing.fetch_objects: Scan used in composite generation - UUID: {fname}, Node: {node}, Nominal date and time: {obj.date}T{obj.time}"
                )
                self.add_how_task_from_scan(obj, tasks)
            elif is_pvol:
                self.logger.debug(
                    f"[{self.mpname}] compositing.fetch_objects: PVOL used in composite generation - UUID: {fname}, Node: {node}, Nominal date and time: {obj.date}T{obj.time}"
                )
    ...
    def get_backup_gra_coefficient(self, db, agedt, nowdt):
        ...
        logger.warning(
            f"[{self.mpname}] compositing.get_backup_gra_coefficient: Could not aquire coefficients newer than {dtstr}, defaulting to climatologic"
        )
    ...
    def _apply_gra(self, result, d, t):
        if nodomdb:
            self.logger.info(
                f"[{self.mpname}] compositing._apply_gra: Could not load rave_dom_db, probably due to missing dependencies like jprops or sqlalchemy, ignoring gra correction"
            )
            return
        ...
        try:
            ...
            if grac != None and not math.isnan(grac.a) and not math.isnan(grac.b) and not math.isnan(grac.c):
                if self.verbose:
                    self.logger.debug(
                        f"[{self.mpname}] compositing._apply_gra: Applying gra coefficients from database"
                    )
                gra.A = grac.a
                gra.B = grac.b
                gra.C = grac.c
                ...
                self.logger.info(
                    f"[{self.mpname}] compositing._apply_gra: Could not find coefficients for given time, trying to get aged or climatologic coefficients"
                )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        "--area",
        dest="area",
        help="Name of Cartesian area to which to generate the composite. If not specified, a best fit "
    ...
    parser.add_option(
        ...
        dest="pcsid",
        default="gmaps",
        help="Name of the pcsid to use if the area should be automatically generated from a best fit. "
        "Default is 'gmaps'.",
    )
    ...
    parser.add_option(
        ...
        type="float",
        default=1000.0,
        help="Product parameter. For (P)CAPPIs it is the height of the desired layer. For PPIs, "
        "it is the elevation angle. Default=1000.0 (meters).",
    )
    ...
    parser.add_option(
        ...
        type="float",
        default=-30.0,
        help="Minimum value that can be represented in composite. Relevant when interpolation is "
        "performed. Default=-30.0",
    )
    ...
    parser.add_option(
        ...
        help="Compositing algorithm to apply. Current choices are NEAREST_RADAR or "
        "HEIGHT_ABOVE_SEALEVEL. Default=NEAREST_RADAR.",
    )
    ...
    parser.add_option(
        ...
        action="store_true",
        dest="ignore_malfunc",
        help="If scans/volumes contain malfunc information. Don't use them in the composite. Default is "
        "to always use everything.",
    )
