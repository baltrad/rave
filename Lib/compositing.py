...
class compositing(object):
    ...
    def _generate(self, dd, dt, area=None):
        ...
        if all_files_malfunc:
            self.logger.info("Content of all provided files were marked as 'malfunc'. "
                             "Since option 'ignore_malfunc' is set, no composite is generated!")
            return None
        ...
        if self.applygra:
            if not "se.smhi.composite.distance.radar" in qfields:
                self.logger.info(
                    "Trying to apply GRA analysis without specifying a quality plugin specifying the "
                    "se.smhi.composite.distance.radar q-field, disabling...")
            else:
                if self.verbose:
    ...
    def set_interpolation_method_from_string(self, methstr):
        if methstr.upper() == "NEAREST_VALUE":
            ...
            raise ValueError(
                "Only supported interpolation methods are NEAREST_VALUE, LINEAR_HEIGHT, LINEAR_RANGE, LINEAR_AZIMUTH, "
                "LINEAR_RANGE_AND_AZIMUTH, LINEAR_3D, QUADRATIC_HEIGHT or QUADRATIC_3D")
    ...
    def get_backup_gra_coefficient(self, db, agedt, nowdt):
        ...
        logger.warn(
            "Could not aquire coefficients newer than %s, defaulting to climatologic"
            % agedt.strftime("%Y%m%d %H:%M:%S")
        )
    ...
    def _apply_gra(self, result, d, t):
        if nodomdb:
            self.logger.info(
                "Could not load rave_dom_db, probably due to missing dependencies like jprops or sqlalchemy, "
                "ignoring gra correction"
            )
            return
...
if __name__ == "__main__":
    ...
    parser.add_option("-s", "--scale", dest="scale",
                      ...
                      help="The x/y-scale to use if the area should be automatically generated from a best fit. "
                           "Default is 2000.0.")
    ...
    parser.add_option("-d", "--date", dest="date",
                      default=None,
                      help="Nominal date of the composite to be written. Defaults to the nominal date of the last "
                           "input file.")
    
    parser.add_option("-t", "--time", dest="time",
                      default=None,
                      help="Nominal time of the composite to be written. Defaults to the nominal time of the last "
                           "input file.")
    ...
    parser.add_option("-I", "--interpolation_method", dest="interpolation_method",
                      type="choice", choices=["NEAREST_VALUE", "LINEAR_HEIGHT", "LINEAR_RANGE", "LINEAR_AZIMUTH",
                                              "LINEAR_RANGE_AND_AZIMUTH", "LINEAR_3D", "QUADRATIC_HEIGHT",
                                              "QUADRATIC_3D"], default="NEAREST_VALUE",
