...
public:
	...
	PolarAttenuation(const string p="1.12e-4,0.62,0,0"){
...
void PolarAttenuation<T,T2>::filter(const image::Image<T> &src,image::Image<T2> &dst) const
{
	const string & cStr = this->parameters.get("c","1.12e-4");
	...
	cout << "Attn: << " << src.getGeometry() << "\n";
	cout << "Attn: << " << dst.getGeometry() << "\n";
	...
	for (unsigned int k = 0; k < srcChannels; ++k) {
		for (unsigned int j = 0; j < srcHeight; ++j) {
			...
			for (unsigned int i = 0; i < srcWidth; ++i) {
				...
				zTrue = drain::radar::dbzToZ( dbzObs ) + zAttnCumulated;
