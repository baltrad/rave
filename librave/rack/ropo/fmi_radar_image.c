...
void detect_emitters2old(FmiImage *source,FmiImage *trace,int min_intensity,int min_length,int max_width){
  register int i,j;
  Byte row_nonzero[source->height];
  Byte row_avg[source->height];
  Byte row_pow[source->height];
  //  FmiImage unity_width_segments;
  FmiImage candidate;
  FILE *fp; // debug
  FmiImage temp;
...
void detect_ships(FmiImage *source,FmiImage *prob,int min_intensity,int max_area){
  ...
  */
  ...
  //  fmi_debug(2,"--------------");
  /* SPECKS */
  // TO DO rel DBZ + ABS DBZ

  // pipeline_process(source,&temp1,max_radius+2,max_radius+2,histogram_mean);
  // subtract_image(source,&temp1,&temp1); 
  // average_images(source,&temp1,&temp1); 
  // if (FMI_DEBUG(2)) write_image("ship_temp",&temp1,PGM_RAW);

  // detect_specks(&temp1,&specks,min_intensity,histogram_area);
  // invert_image(&specks); /* to get small specks the brightest */
  // translate_intensity(&specks,255,0);
  // threshold_image(&specks,&specks,255-max_area);
  // if (FMI_DEBUG(2)) write_image("ship_speck",&specks,PGM_RAW);
  ...
  //  multiply_image_scalar(prob,512); /* don't ask about 200 */
...
void detect_ground_echo_mingrad(FmiImage *source,int ppi_count,FmiImage *prob,int intensity_grad,int half_altitude){
  ...
  const int image_height=source[0].height;
  const int image_width=source[0].width;
  //  int g,g_min,g_max,g_max_alt,g_span_max;
  // int grad_min;
  int p; //,p_min;
  // float beta_l; /* ANGLE OF THE LOWER  SWEEP */
  // float beta_h; /* ANGLE OF THE HIGHER SWEEP */
...
/*
  ...
  int i,j;
  unsigned char map[RGBCOUNT][4];
  fmi_debug(1,"byte_to_rgb (mapping from 1 -> 3 rgb channels)");
  //  copy_image_properties(source,target);
  //target->channel=1;
  if (source->channels!=1)
    fmi_error("byte_to_rgb: other than single-channel source");
  for (i=0;i<RGBCOUNT;i++){
    map[i][0]= DBZ_TO_BYTE(dbz_rgb[i][0]);
    for (j=1;j<4;j++) map[i][j]=dbz_rgb[i][j];}
  map_channel_to_colors(source,0,target,RGBCOUNT,map);
  return 1;
}
*/
